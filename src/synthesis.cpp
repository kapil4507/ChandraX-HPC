...
bool synthesizeImage(const ComplexFloat* h_data, int lines, int samples, const string& outputPath, double& synthesisSecs) {
    ...
    cout << "[Phase 4] Starting magnitude calculation and 2.5-sigma contrast stretching..." << endl;

    vector<float> magnitudes(numElements);
    double sum = 0.0;
    ...
    for (size_t i = 0; i < numElements; ++i) {
        ...
        float mag = sqrt(realVal * realVal + imagVal * imagVal) / lines;
        magnitudes[i] = mag;
        sum += mag;
    }
    ...
    float stddev = sqrt(static_cast<float>(sq_sum / numElements));
    ...
    vector<unsigned char> pixels(numElements);
